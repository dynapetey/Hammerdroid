...
class ObdxGtClient(private val transport: BluetoothSppTransport) {
    suspend fun initialize(log: (String) -> Unit): GtInfo {
        transport.write(byteArrayOf(0x25, 0x00, 0xDA.toByte()))
        delay(250)
        transport.discardInput()
        elm("ATZ", 700)
        val description = elm("AT@1", 1000)
        require(description.contains("OBDX", true)) { "Selected device is not an OBDX adapter" }
        require(elm("DXDP1", 1000).contains("OK", true)) { "GT refused DVI mode" }
        ...
        val fw = versionData(transact(0x22, byteArrayOf(0x01)), 0x01)
        val hw = versionData(transact(0x22, byteArrayOf(0x00)), 0x00)
        ...
        return GtInfo(name, fw, hw, serial, raw * 0.009047468 + 0.2)
    }

    suspend fun identifyPcm(log: (String) -> Unit): PcmIdentity {
        log("Querying PCM…")
    ...
    suspend fun checkConnection(): Boolean = runCatching {
        transport.checkConnected()
        asciiData(transact(0x22, byteArrayOf(0x03), timeoutMs = 800), 0x03)
    ...
    private suspend fun elm(command: String, timeout: Long): String {
        transport.write("$command\r".toByteArray(StandardCharsets.US_ASCII))
        delay(60)
        return transport.readAvailable(totalMs = timeout)
    ...
    private suspend fun readPacket(timeout: Long): DviPacket {
        val command = transport.readByte(timeout)
        val lengthHighOrShort = transport.readByte(timeout)
        require(command >= 0 && lengthHighOrShort >= 0) { "Timed out waiting for GT" }
        val header: ByteArray
        val length: Int
        if (command == 0x09) {
            val lengthLow = transport.readByte(timeout)
            require(lengthLow >= 0) { "Timed out waiting for GT packet length" }
            length = (lengthHighOrShort shl 8) or lengthLow
            header = byteArrayOf(
                command.toByte(),
                lengthHighOrShort.toByte(),
                lengthLow.toByte()
            )
        } else {
        ...
        body.indices.forEach {
            body[it] = transport.readByte(timeout)
                .also { byte -> require(byte >= 0) }
                .toByte()
        }
        val complete = header + body
        require(checksum(complete, complete.lastIndex) == complete.last()) {
            "DVI checksum failed"
        }
    ...
    private suspend fun sendVpw(message: ByteArray): ByteArray {
        ...
        while (System.currentTimeMillis() < end) {
            ...
            val packet = try {
                withTimeout(remaining.coerceAtLeast(1)) {
                    readPacket(remaining.coerceAtMost(600).coerceAtLeast(1))
                }
            } catch (_: TimeoutCancellationException) {
                continue
            } catch (error: IllegalArgumentException) {
                if (error.message?.startsWith("Timed out") == true) {
                    continue
                } else {
                    throw error
                }
    ...
    private fun isMatchingVpwResponse(
        request: ByteArray,
        response: ByteArray
    ): Boolean {
        ...
        return request.size < 5 ||
            (response.size >= 5 && response[4] == request[4])
    }

    private fun isCorrelatedNegativeResponse(
        request: ByteArray,
        response: ByteArray
    ): Boolean =
        response.size >= 5 &&
